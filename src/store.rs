...
use crate::types::{
    CommitReceipt, Direction, Node, SearchBackend, SearchFilter, SearchResults, WriteBatch,
};
    ...
    pub fn open(
        ...
        if stored_dimensions != dimensions {
            return Err(Error::InvalidInput(format!(
                "embedding dimension mismatch: stored {stored_dimensions}, requested {dimensions}"
            )));
        }

        let vector_path = database_path.with_extension("usearch");
        let accelerator = VectorAccelerator::open_or_rebuild(
            &connection,
            vector_path.clone(),
            dimensions,
            generation,
